...
class MCPServer:
    ...
    def __init__(self, workspace_path: str = './mcp_workspace') -> None:
        """Initialize the MCP server and create the workspace directory.
    ...
    def list_files(self) -> list[str] | str:
        ...
        Returns:
            List of filenames, or 'No files found' if the workspace is empty.
        """
    ...
    def calculate_statistics(self, numbers: list[float]) -> dict[str, float | int] | str:
        ...
        Returns:
            Dictionary with count, mean, median, min, and max, or an error message.
        """
        if not numbers:
...
def run_server(
    ...
    workspace: str = './mcp_workspace',
    max_iterations: int = 5,
) -> str:
    """Run the MCP server and execute a query using available tools.
